...
@rom_bp.route('/view/rom', methods=['GET'])
@login_required
def view_rom():
    try:
        conn = get_connection()
        search = request.args.get('search', '').strip()
        query = "SELECT * FROM azr.INS_ROM"
        params = []

        # Filter user non-admin berdasarkan NRP
        if not current_user.is_admin:
            query += " WHERE NRP = ?"
            params.append(current_user.get_id())  # gunakan NRP sebagai ID login
        ...
        print("[ERROR VIEW ROM]", e)
        flash(f"Gagal mengambil data ROM: {e}", "danger")
